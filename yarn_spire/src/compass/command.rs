...
///
/// This variant is returned when the coroutine yields a `Command` instruction.
///
/// It ensures that you do not mistakenly provide a player decision when continuing the dialogue.
///
/// You can do so by calling [next](crate::prelude::CommandCompass::next).
pub struct CommandCompass<'a,
...
impl<'a,
     ...
     TCmd: Clone + PartialEq + Debug + Serialize + DeserializeOwned,
     ID: NodeID<Storage = TStorage, Command = TCmd>>
CommandCompass<'a, TStorage, TCmd, ID> {
	...
	/// The command yielded when this compass was created.
	pub fn command(&self) -> &TCmd {
		return &self.command;
	}
	...
	/// can break the determinism necessary for serialization/deserialization.
	pub unsafe fn deconstruct(self) -> (Box<TStorage>, BoxedYarnNode<'a, TStorage, TCmd, ID>, TCmd) {
		return (self.storage, self.node, self.command);
	}
