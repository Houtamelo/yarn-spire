...
use crate::io::write::util::{get_or_create_file, write_to_file, delete_file_if_exists};
...
fn write_all_nodes_root(cfg: &YarnConfig, nodes: &[IDNode]) -> Result<()> {
	let tokens =
		nodes::tokens_all_nodes_root(nodes);
	let path =
		cfg.destination_os_path.join("nodes/mod.rs");
	let file =
...
fn write_node_specific_roots(cfg: &YarnConfig, nodes_mapped: &[(&IDNode, LinesMap)]) -> Result<()> {
	nodes_mapped
		...
		.try_for_each(|(node, lines_map)| {
			let tokens =
				nodes::tokens_node_root(node, lines_map);
			let path =
				cfg.destination_os_path.join(format!("nodes/{title}/mod.rs", title = &node.metadata.title));
			let file =
				get_or_create_file(&path, cfg.allow_overwrite)?;
...
fn write_title_modules(cfg: &YarnConfig, nodes: &[IDNode]) -> Result<()> {
	let inferred_tracking = 
		title::infer_all_nodes_tracking(nodes)?;
	
	inferred_tracking
		.into_iter()
		.try_for_each(|(node, tracking)| {
			let tokens =
				title::all_tokens(cfg, node, tracking);
			let path =
				cfg.destination_os_path.join(format!("nodes/{title}/title.rs", title = &node.metadata.title));
			let file =
				get_or_create_file(&path, cfg.allow_overwrite)?;
...
fn write_enum_any_modules(cfg: &YarnConfig,
	...
	nodes_mapped
		...
		.try_for_each(|(node, lines_map)| {
			let path =
				cfg.destination_os_path.join(format!("nodes/{title}/enum_any.rs", title = &node.metadata.title));
...
fn write_enum_command_modules(cfg: &YarnConfig,
	...
	nodes_mapped
		...
		.try_for_each(|(node, lines_map)| {
			let path =
				cfg.destination_os_path.join(format!("nodes/{title}/enum_command.rs", title = &node.metadata.title));
...
fn write_enum_options_fork_modules(cfg: &YarnConfig,
	...
	nodes_mapped
		...
		.try_for_each(|(node, lines_map)| {
			let path =
				cfg.destination_os_path.join(format!("nodes/{title}/enum_options_fork.rs", title = &node.metadata.title));
...
fn write_enum_speech_modules(cfg: &YarnConfig,
	...
	nodes_mapped
		...
		.try_for_each(|(node, lines_map)| {
			let path =
				cfg.destination_os_path.join(format!("nodes/{title}/enum_speech.rs", title = &node.metadata.title));
...
fn write_enum_option_line_modules(cfg: &YarnConfig,
	...
	nodes_mapped
		...
		.try_for_each(|(node, lines_map)| {
			let path =
				cfg.destination_os_path.join(format!("nodes/{title}/enum_option_line.rs", title = &node.metadata.title));
