...
use crate::quoting::quotable_types::line_ids::{BuiltInCommand, convert_to_id_nodes, IDFlatLine};
...
fn check_nodes_in_jumps(nodes: &[IDNode]) -> Result<()> {
		...
		nodes.iter()
			 .flat_map(|node| 
				 node.scopes
					 ...
					 .filter_map(|line|
						 if let IDFlatLine::BuiltInCommand(BuiltInCommand::Jump { node_destination_title, .. }) 
							 = line {
	...
	return if nodes_not_found.is_empty() {
		Ok(())
	} else {
		Err(anyhow!(
			"Some nodes mentioned in `jump`(`<<jump [node_name]>>`) commands are not present in the provided files.\n\
			 Node names: {}\n\
			 Make sure that the node names are correct."
			, nodes_not_found.join(", ")))
	};
}
