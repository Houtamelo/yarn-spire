		...
		pub(crate) trait OptionLineTrait {
			...
			fn text(&self, storage: &$(&cfg.storage_direct)) -> Cow<'static, str>;
			 ...
			 $(Comments([
				...
				r#"When reaching this line, the variable `player_has_parachute` will be fetched from the [VariableStorage](crate::shared_internal::VariableStorage),"#,
				r#"making `is_available` be `Some(storage.get_var::<player_has_parachute>())`."#,
				r#"Note that, in this case, it is expected for the return value of `get_var::<player_has_parachute>()` to be a boolean, 
...
fn tokens_enums(nodes: &[IDNode]) -> Tokens {
	let forks =
		nodes.iter()
		     .map(|node| {
