...
use super::{parse_raw_nodes, RawLine};
use super::Content;
...
use crate::parsing::raw::branches::if_statement::{ElseIfStruct, ElseStruct, EndIfStruct, IfStruct};
...
fn test_parsing() {
	...
	let raw_lines = 
		parse_raw_nodes(unparsed_lines)
			.unwrap();
	...
	assert_eq!(raw_lines[7], RawLine { indent: 0, content: Content::OptionLine(
		OptionLine { line_number: 7, text: (own!("Option A Do that"), vec![]), if_condition: None, metadata: None }) });
	assert_eq!(raw_lines[8], RawLine { indent: 10, content: Content::OptionLine(
		OptionLine { line_number: 8, text: (own!("Option B Do this"), vec![]), if_condition: None, metadata: Some(own!("with tags")) }) });
