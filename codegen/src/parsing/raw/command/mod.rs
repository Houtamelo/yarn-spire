...
fn parse_command_name(args_iter: &mut ArgsIter) -> Result<String> {
	let expr = args_iter
		.next()
		.unwrap_or_else(|| Err(anyhow!("Expected `Rust identifier` argument but `{}` returned None.",
			type_name::<ArgsIter>())))?;

	if let YarnExpr::Identifier(cmd_name) = expr {
		Ok(cmd_name)
	} else {
		Err(anyhow!(
...
fn parse_set_command(remaining_line: String) -> Result<(String, SetOperation, YarnExpr)> {
	let mut remaining_str = remaining_line.trim();
	...
	let mut remaining_str = rest.trim();
	...
	let value_expr = parse_yarn_expr(remaining_str)
		.map_err(|err| anyhow!(
	...
	Ok((var_name.to_string(), operation, value_expr))
}
...
fn parse_jump_command(args_iter: &mut ArgsIter) -> Result<String> {
	...
	match expr {
		...
		_ => {
			Err(anyhow!("Expected `node name` argument to be `YarnExpr::Lit(YarnLit::Str(node_name))` or `YarnExpr::Identifier(node_name)`.\n\
						 Instead got: `{expr:?}`\n\n\
