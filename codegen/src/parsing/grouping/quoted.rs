...
use crate::expressions::yarn_expr::{BuiltInFunctionCall, YarnExpr};
	...
	fn format_into(self, tokens: &mut Tokens<Rust>) {
		match self {
			...
			BuiltInFunctionCall::Random => {
				quote_in!(*tokens => built_in_functions::random())
			},
			BuiltInFunctionCall::RandomRange(min_expr, max_expr) => {
				quote_in!(*tokens => built_in_functions::random_range($(min_expr.as_ref()).into(), $(max_expr.as_ref()).into()))
			},
			BuiltInFunctionCall::Dice(expr) => {
				quote_in!(*tokens => built_in_functions::dice($(expr.as_ref()).into()))
			},
...
/*
impl Quoted for OptionLine {
	fn quoted(&self, storage_ident: &Ident, command_ident: &Ident)
	          -> Result<TokenStream> {
		let text_tokens = {
			let (literal, args) = &self.text;
...
impl Quoted for FlatLine {
	fn quoted(&self, _storage_ident: &Ident, _command_ident: &Ident)
	          -> Result<TokenStream> {
		return Err(anyhow!(""));
		/*
...
impl Quoted for OptionsFork {
	fn quoted(&self, storage_ident: &Ident, command_ident: &Ident)
	          -> Result<TokenStream> {
		let (first_line, first_scope,
			other_lines, other_scopes) = {
			let mut options =
				self.iter_options();
			...
			let (other_lines, other_scopes): (Vec<&OptionLine>, Vec<&Option<Box<YarnScope>>>) =
				options.map(|tuple| (&tuple.0, &tuple.1)).unzip();
			
			(first_line, first_scope, other_lines, other_scopes)
		};
		
		let (first_line_tokens, first_scope_tokens) = {
			let line_tokens =
				first_line.quoted(storage_ident, command_ident)?;
			
			let scope_inner_tokens = 
				first_scope.quoted(storage_ident, command_ident)?;
			...
			(line_tokens, scope_tokens)
		};
		
		let other_lines_tokens =
			other_lines
				.into_iter()
				.map(|option| option.quoted(storage_ident, command_ident))
				.collect::<Result<Vec<TokenStream>>>()?;

		let enumerated_scopes: Vec<TokenStream> =
			other_scopes
				.into_iter()
				.enumerate()
				.map(|(index, scope_option)| 
					(index + 1, scope_option))
				.map(|(index, scope_option)| {
					let inner_tokens =
						scope_option.quoted(storage_ident, command_ident)?;

					let index_literal =
						Literal::usize_unsuffixed(index);
...
impl Quoted for Flow {
	fn quoted(&self, storage_ident: &Ident, command_ident: &Ident)
		...
		return match self {
			...
			Flow::OptionsFork(choices) => choices.quoted(storage_ident, command_ident),
