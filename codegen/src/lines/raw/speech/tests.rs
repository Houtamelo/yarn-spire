...
use crate::lines::raw::{Content, ParseRawYarn};
use crate::lines::raw::speech::{Speech, Speaker};
...
fn test_parse_dialogue_with_speaker_and_line() {
	...
	let expected = Speech {
		...
		line: (own!("This is the dialogue line"), vec![]),
		metadata: None,
	};
...
fn test_parse_dialogue_with_speaker_var_and_args() {
	...
	let expected = Speech {
		line_number: 0,
		speaker: Some(Speaker::Variable(own!("$player_name"))),
		line: (own!("This is the dialogue line"), 
			vec![
				YarnExpr::BinaryOp {
...
fn test_parse_dialogue_with_colon_in_line() {
	...
	let expected = Speech {
		line_number: 0,
		speaker: None,
		line: (own!("This is the dialogue line with a colon: in it"), vec![]),
		metadata: None,
	};
	let result = Speech::parse_raw_yarn(line, 0).unwrap();
	assert_eq!(result, Ok(expected));
	...
	assert_eq!(Speech::parse_raw_yarn(source_text[5], 5).unwrap(),
		Content::Speech(
			Speech{
				line_number: 5,
				speaker: None,
				line: (own!("\tYou wake up. Something you shouldn't have done."), Vec::new()),
				metadata: None,
			}));
	...
	assert_eq!(Speech::parse_raw_yarn(source_text[6], 6).unwrap(),
		Content::Speech(
			Speech {
				...
				line: (own!("Ethel: hey there, {} after var"), vec![YarnExpr::VarGet(own!("player_name"))]),
				metadata: None,
			}));
	...
	assert_eq!(Speech::parse_raw_yarn(source_text[16], 16).unwrap(),
		Content::Speech(
			Speech {
				speaker: Some(Speaker::Literal(own!("Ethel"))),
				line_number: 16,
				line: (own!("Ethel: hey there, I played this game {} times!"),
					vec![
						YarnExpr::BinaryOp {
