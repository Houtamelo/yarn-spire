...
use crate::lines::raw::branches::if_statement::{BranchKind, ElseIfStruct, ElseStruct, EndIfStruct, IfStruct};
use std::str::FromStr;
...
fn test_no_if() {
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> hello", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
			if_condition: None,
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> hello # metadata", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
			if_condition: None,
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> hello# test data", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
			if_condition: None,
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("->Very big choice my dude \"not metadata or \\#\\# tags\"", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("Very big choice my dude \"not metadata or ## tags\""), vec![]),
			if_condition: None,
			metadata: None,
		});

	assert_eq!(ChoiceOption::parse_raw_yarn("->Very big choice my dude \"not metadata ## tags\" # actual metadata here, cant escape this #", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("Very big choice my dude \"not metadata ## tags\""), vec![]),
			if_condition: None,
...
fn test_with_if() {
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> <<if $condition>> hello", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> <<if $condition>> hello # metadata", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
			if_condition: Some(YarnExpr::VarGet(own!("condition"))),
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> <<if $condition>> hello # metadata # more metadata", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
			if_condition: Some(YarnExpr::VarGet(own!("condition"))),
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> <<if 5 > 3>> hello # metadata # more metadata", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
	...
	assert_eq!(ChoiceOption::parse_raw_yarn("-> <<if {(5 > 3) / 3 + 2}>> hello # metadata # more metadata", 0).unwrap(),
		ChoiceOption {
			line_number: 0,
			line: (own!("hello"), vec![]),
			if_condition: Some(YarnExpr::BinaryOp {
...
fn test_invalid() {
	...
	assert_matches!(ChoiceOption::parse_raw_yarn("-> <<if $condition>>", 0).unwrap(),
		Err(_));

	assert_matches!(ChoiceOption::parse_raw_yarn("-> <<if $condition>> # metadata", 0).unwrap(),
		Err(_));

	assert_matches!(ChoiceOption::parse_raw_yarn("-> Hey there <<if $condition # metadata", 0).unwrap(),
		Err(_));

	assert_matches!(ChoiceOption::parse_raw_yarn("-> Hey there <<if $condition>> metadata", 0).unwrap(),
		Err(_));

	assert_matches!(ChoiceOption::parse_raw_yarn("-> Hey there <<if >> #metadata", 0).unwrap(),
		Err(_));

	assert_matches!(ChoiceOption::parse_raw_yarn("-> Hey there <<$condition>> metadata", 0).unwrap(),
		Err(_));

	assert_matches!(ChoiceOption::parse_raw_yarn("-> Hey there <<not_if $condition>> #metadata", 0).unwrap(),
		Err(_));
}
