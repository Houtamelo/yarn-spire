...
use crate::lines::raw::branches::if_statement::{ElseIfStruct, EndIfStruct, IfStruct};
use crate::lines::raw::command::YarnCommand;
use crate::lines::raw::speech::{Speaker, Speech};
...
use crate::lines::raw::node_metadata::{NodeMetadata, TrackingSetting};
use crate::lines::raw::{Content, RawLine, RawNode};
...
pub fn node_1_raw() -> RawNode {
	RawNode {
		...
		lines: vec![
			...
			RawLine { indent: 0, 
				content: Content::ChoiceOption(
					ChoiceOption {
						line_number: 7,
						line: (own!("Continue Sleeping"), vec![]),
						if_condition: None,
						metadata: None
					})
			},
			...
			RawLine { indent: 8, 
				content: Content::Speech(
					Speech {
						line_number: 10,
						speaker: None,
						line: (own!("You managed to get up."), vec![]),
						metadata: None,
					})
			},
			RawLine { indent: 4,
				content: Content::ElseIf(
					ElseIfStruct {
						line_number: 11,
						condition: YarnExpr::FunctionCall {
			...
			RawLine { indent: 8, 
				content: Content::Speech(
					Speech {
						line_number: 12,
						speaker: None,
						line: (own!("You managed to get up, but it was hard."), vec![]),
						metadata: None,
					})
			},
			RawLine { indent: 4, 
			...
			RawLine { indent: 8, 
				content: Content::Speech(
					Speech {
						line_number: 14,
						speaker: None,
						line: (own!("You failed to get up."), vec![]),
						metadata: None,
					})
			},
			RawLine { indent: 4, 
			...
			RawLine { indent: 0, 
				content: Content::Speech(
					Speech {
						...
						line: (own!("The end. Here is your score: {$score}."), vec![]),
						metadata: None,
					})
			},
		],
				...
				Flow::Flat(vec![
					FlatLine::Speech(
						Speech {
							line_number: 10,
							speaker: Some(Speaker::Literal(own!("Narrator"))),
							line: (own!("The end. Here is your score: {}."), vec![YarnExpr::VarGet(own!("score"))]),
							metadata: None,
						}
					),
				])
